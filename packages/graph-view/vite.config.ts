...
const ACCESS_TOKEN = process.env.NOVADIFF_GRAPH_ACCESS_TOKEN || crypto.randomBytes(16).toString("hex");
...
function graphFileCandidates(fileName: string): string[] {
  ...
  return [
    ...(graphDir
      ? [path.resolve(graphDir, `.novadiff-graph/${fileName}`)]
      : []),
    path.resolve(process.cwd(), `.novadiff-graph/${fileName}`),
    path.resolve(process.cwd(), `../../../.novadiff-graph/${fileName}`),
  ];
}
...
function readSourceFile(url: URL) {
  ...
  if (!graphFile) {
    return rejectFileRequest("No knowledge graph found. Build the graph in NovaDiff first.", 404);
  }
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      "@novadiff/graph-core/schema": path.resolve(__dirname, "../core/dist/schema.js"),
      "@novadiff/graph-core/search": path.resolve(__dirname, "../core/dist/search.js"),
      "@novadiff/graph-core/types": path.resolve(__dirname, "../core/dist/types.js"),
    },
  },
        ...
        server.middlewares.use((req, res, next) => {
          ...
          for (const candidate of candidates) {
            ...
            try {
              ...
              console.error("[novadiff-graph] Failed to sanitise graph file:", err);
              res.statusCode = 500;
              res.setHeader("Content-Type", "application/json");
          ...
          if (pathname === "/knowledge-graph.json") {
            res.setHeader("Content-Type", "application/json");
            res.end(JSON.stringify({ error: "No knowledge graph found. Build the graph in NovaDiff first." }));
          } else {
            res.end();
          }
        });
