...
const EMPTY_OVERVIEW = { nodes: [] as Node[], edges: [] as Edge[] };
...
function useOverviewGraph() {
  ...
  const searchResultIds = useMemo(
    () => searchResults.map((r) => r.nodeId).join("\n"),
    [searchResults],
  );
  ...
  const built = useMemo(() => {
    ...
    if (layers.length === 0 && nodesById.size > 0) {
      const byLayer = new Map<string, string[]>();
      for (const node of nodesById.values()) {
        if (!node.layerId) {
          continue;
        }
        const list = byLayer.get(node.layerId) ?? [];
        list.push(node.id);
        byLayer.set(node.layerId, list);
      }
      layers = [...byLayer.entries()].map(([id, nodeIds]) => ({
        id,
        name: id.replace(/^layer:/, "") || "root",
  ...
  }, [graph, nodesById, nodeIdToLayerId, searchResultIds, drillIntoLayer, searchResults]);

  const [overview, setOverview] = useState<{ nodes: Node[]; edges: Edge[] }>(EMPTY_OVERVIEW);
  const [layoutStatus, setLayoutStatus] = useState<"computing" | "ready">("ready");

  useEffect(() => {
    if (!built) {
      setOverview((prev) =>
        prev.nodes.length === 0 && prev.edges.length === 0 ? prev : EMPTY_OVERVIEW,
      );
...
export function GraphViewInner() {
  ...
  // onNodesChange (measure/fitView); resetting on position drift causes an
  ...
  useEffect(() => {
    if (initialEdgesRef.current === initialEdges) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (!layoutReady || initialNodes.length === 0) {
      return;
    }
  ...
  return (
    <div className="h-full w-full relative">
      ...
      <ReactFlow
        ...
        onMove={!embedMode && navigationLevel === "layer-detail" ? onMove : undefined}
