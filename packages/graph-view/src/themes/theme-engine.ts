...
/** Theme target: document root for standalone graph-view, host element when embedded in NovaDiff. */
export function applyTheme(
  ...
  target.setAttribute("data-theme", preset.isDark ? "dark" : "light");
...
export function clearTheme(target: HTMLElement, config: ThemeConfig): void {
  ...
  for (const key of Object.keys(preset.colors)) {
    style.removeProperty(`--color-${key}`);
  }
  style.removeProperty("--color-accent");
  style.removeProperty("--color-accent-dim");
  style.removeProperty("--color-accent-bright");
