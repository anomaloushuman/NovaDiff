...
import { SearchEngine } from "@novadiff/graph-core/search";
import type { SearchResult } from "@novadiff/graph-core/search";
...
interface DashboardStore {
  ...
  /** Close transient UI (modals, code viewer) without toggling — safe for embed graph load. */
  resetEmbedOverlays: () => void;
...
export const useDashboardStore = create<DashboardStore>()((set, get) => ({
  ...
  navigateToOverview: () => {
    const state = get();
    if (
      state.navigationLevel === "overview" &&
      state.activeLayerId === null &&
      state.selectedNodeId === null &&
      state.focusNodeId === null &&
      !state.codeViewerOpen &&
      state.expandedContainers.size === 0 &&
      state.pendingFocusContainer === null
    ) {
      return;
    }
    set({
  ...
  setDiffOverlay: (changed, affected) =>
    set((state) => {
      const sameSize =
        state.changedNodeIds.size === changed.length &&
        state.affectedNodeIds.size === affected.length;
      const sameChanged =
        sameSize && changed.every((id) => state.changedNodeIds.has(id));
      const sameAffected =
        sameSize && affected.every((id) => state.affectedNodeIds.has(id));
      if (state.diffMode && sameChanged && sameAffected) {
        return {};
      }
      return {
        diffMode: true,
