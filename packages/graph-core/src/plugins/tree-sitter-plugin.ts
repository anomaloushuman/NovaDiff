...
import { builtinExtractors, genericExtractor } from "./extractors/index.js";
...
export class TreeSitterPlugin implements AnalyzerPlugin {
    ...
    if (this.configs.length > 0) {
      ...
      for (const config of this.configs) {
        ...
        const loadGrammar = async () => {
          try {
            const ts = config.treeSitter!;
            const wasmPath =
              "wasmsFile" in ts
                ? require.resolve(`tree-sitter-wasms/out/${ts.wasmsFile}`)
                : require.resolve(`${ts.wasmPackage}/${ts.wasmFile}`);
              ...
              try {
                const tsxWasm =
                  "wasmsFile" in ts
                    ...
                    : require.resolve(`${ts.wasmPackage}/tree-sitter-tsx.wasm`);
  ...
  analyzeFile(
    ...
    result = extractor
      ...
      : genericExtractor.extractStructure(tree.rootNode);
