  ...
  if (Array.isArray(data.nodes)) {
    result.nodes = (data.nodes as Record<string, unknown>[]).map((node, i) => {
      ...
      if (typeof n.summary !== "string" || !n.summary.trim()) {
        const fallback =
          typeof n.filePath === "string" && n.filePath.trim()
            ? `${(n.name as string) || "node"} — ${n.filePath}`
            : (n.name as string) || "No summary";
        n.summary = fallback;
        issues.push({
          level: "auto-corrected",
          category: "missing-field",
...
export function validateGraph(data: unknown): ValidationResult {
  ...
  // Repair layers missing nodeIds (NovaDiff builder used to set layerId on nodes only).
  if (Array.isArray(fixed.layers)) {
    const byLayerId = new Map<string, string[]>();
    for (const node of validNodes) {
      const layerId =
        typeof node.layerId === "string" && node.layerId.trim() ? node.layerId : null;
      if (!layerId) {
        continue;
      }
      const list = byLayerId.get(layerId) ?? [];
      list.push(node.id);
      byLayerId.set(layerId, list);
    }
    fixed.layers = (fixed.layers as unknown[]).map((raw) => {
      const layer = raw as Record<string, unknown>;
      const id = String(layer.id ?? "");
      const existing = Array.isArray(layer.nodeIds)
        ? (layer.nodeIds as string[]).filter((nid) => nodeIds.has(nid))
        : [];
      const fromNodes = (byLayerId.get(id) ?? []).filter((nid) => nodeIds.has(nid));
      const merged = [...new Set([...existing, ...fromNodes])];
      if (merged.length > existing.length && id) {
        issues.push({
          level: "auto-corrected",
          category: "layer-nodeIds",
          message: `layers["${id}"]: filled nodeIds from node.layerId (${merged.length} nodes)`,
          path: `layers.${id}.nodeIds`,
        });
      }
      return { ...layer, nodeIds: merged };
    });
  }
