...
describe("IgnoreFilter", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(testDir, ".novadiff-graph"), { recursive: true });
  });
  ...
  describe("createIgnoreFilter with user .novadiffignore", () => {
    it("reads patterns from .novadiff-graph/.novadiffignore", () => {
      writeFileSync(
        join(testDir, ".novadiff-graph", ".novadiffignore"),
        "# Exclude tests\n__tests__/\n*.test.ts\n"
      );
      const filter = createIgnoreFilter(testDir);
    ...
    it("reads patterns from project root .novadiffignore", () => {
      writeFileSync(
        join(testDir, ".novadiffignore"),
        "docs/\n"
      );
      const filter = createIgnoreFilter(testDir);
    ...
    it("merges .novadiff-graph/ and root .novadiffignore", () => {
      writeFileSync(
        join(testDir, ".novadiff-graph", ".novadiffignore"),
        "__tests__/\n"
      );
