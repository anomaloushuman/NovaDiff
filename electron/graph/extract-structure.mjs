 ...
 * Uses PluginRegistry (TreeSitterPlugin + non-code parsers) from @novadiff/graph-core
...
let core;
try {
  core = await import(pathToFileURL(require.resolve('@novadiff/graph-core')).href);
} catch {
  // Fallback: direct path for installed plugin cache layouts
  core = await import(pathToFileURL(resolve(repoRoot, 'packages/graph-core/dist/index.js')).href);
}
...
async function main() {
  ...
  for (const file of batchFiles) {
    ...
    let analysis = null;
    if (totalLines <= MAX_ANALYZE_LINES) {
      try {
